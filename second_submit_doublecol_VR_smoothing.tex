...
%
\begin{IEEEeqnarray}{rCl}
 \text{NEES}_k & = & (\hat{x}_k - x_k^*)^T \hat{P}_k^{-1} (\hat{x}_k - x_k^*) \nonumber \\
 \hat{x}_k & = & \frac{1}{N_S} \sum_i x_{k}^{(i)} \nonumber \\
 \hat{P}_k & = & \frac{1}{N_S} \sum_i (x_{k}^{(i)} - \hat{x}_k) (x_{k}^{(i)} - \hat{x}_k)^T \nonumber     ,
\end{IEEEeqnarray}
...
%
\begin{IEEEeqnarray}{rCl}
 \text{TNEES}_k & = &\frac{\text{NEES}_k}{1+\text{NEES}_k}     .
\end{IEEEeqnarray}
...
\subfloat[]{\includegraphics[width=0.45\columnwidth]{MTNEES_vs_numpts.pdf}}
