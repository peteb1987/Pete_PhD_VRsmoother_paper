...
%
\begin{IEEEeqnarray}{rCl}
w_n^{(i)} & = & \frac{w_{n-1}^{(i)}}{v_{n-1}^{(i)}} \times p(y_n|\theta_{n}^-, y_{1:n-1}) \label{eq:bootstrap_vrpf_weights}     .
\end{IEEEeqnarray}
...
\bibliography{Cleanbib}

\input{IEEE-Bunch.tex}
\input{IEEE-Godsill.tex}
