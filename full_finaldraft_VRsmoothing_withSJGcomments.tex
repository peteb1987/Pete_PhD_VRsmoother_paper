...
This closely resembles the variable-dimension spaces employed by reversible jump Markov chain Monte Carlo algorithms \cite{Green1995}.% or in multiple target tracking scenarios \cite{Vo2005}.
...
where $\delta_x(X)$ is a unit probability mass at $X=x$.
...
The Rao-Blackwellised VRPS algorithm was tested on the financial time series model of \cite{Christensen2012}, in which prices of an asset are treated as noisy observations of a latent state, which evolves according to a drift-diffusion with occasional jumps.
...
The times between changepoints are assumed to be exponentially distributed with mean $1/\alpha$, and equal probability of value and trend jumps.
%
%\begin{IEEEeqnarray}{rCl}
...
where $\lambda$ is the mean reversion constant, $\sigma$ is the diffusion constant, $\mathbf{\beta}(t)$ is standard Brownian motion, $d\mathbf{J}(t)$ is a jump term defined according to,
%
\begin{IEEEeqnarray}{rCl}
 d\mathbf{J}(t) & = & \begin{cases} \mathbf{J}_k & t \in \{\tau_k\} \\ 0 & \text{elsewhere} \end{cases} \\
 \mathbf{J}_k  & \sim & \mathcal{N}(\mathbf{J}_k| \mathbf{0}, Q_{J,u_k})    .
...
where the $\mathbf{w}_n$ is a Gaussian random variable with covariance matrix $Q_n$. The inter-observation time is denoted $\Delta t = t_{n} - t_{n-1}$, and,
